...
import {
  ...
  UserPlus, KeyRound, Copy, BookOpen,
} from "lucide-react";
...
const STEPS = ["Identidad", "Contacto", "Profesional", "Documentos", "Revisar"];
const DOC_CATS = [
  { key: "hoja_vida", label: "Hoja de vida" },
  { key: "titulo", label: "Título / diplomas" },
  { key: "documento_identidad", label: "Documento de identidad" },
  { key: "otros", label: "Otros soportes" },
];
const SUGGESTED = ["Matemáticas", "Lengua Castellana", "Ciencias Naturales", "Sociales", "Inglés", "Educación Física", "Artes", "Tecnología"];
...
export default function NuevoProfesorPage() {
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 1) return EMAIL_RE.test(email.trim()) && mobile.trim();
    return true;
  }, [step, p.documentType, p.documentId, p.birthDate, firstName, lastName, email, mobile]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await apiPost<{ temporaryPassword?: string }>("/users", {
        email: email.trim(),
        role: "TEACHER",
        firstName: firstName.trim(),
        lastName: lastName.trim(),
        ...(mobile.trim() ? { phone: mobile.trim() } : {}),
        ...(speciality.trim() ? { speciality: speciality.trim() } : {}),
        ...(avatarUrl ? { avatarUrl } : {}),
      ...
      setError(e instanceof Error ? e.message : "No se pudo registrar el docente.");
    ...
    return (
      <div className="mx-auto flex max-w-lg flex-col items-center gap-5 px-8 py-20 text-center">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[13px] text-subtle">Asígnale materias y horario desde <Link href="/horarios" className="font-semibold text-primary">Horarios</Link>.</p>
        </div>
        {done.password && (
        ...
        <div className="flex gap-2">
          <Link href="/profesores" className="flex h-9 items-center rounded-lg border border-line px-4 text-[13px] font-semibold text-ink hover:bg-surface">Ver docentes</Link>
          <button onClick={() => window.location.reload()} className="flex h-9 items-center gap-1.5 rounded-lg bg-primary px-4 text-[13px] font-semibold text-white hover:opacity-90"><UserPlus className="h-3.5 w-3.5" /> Otro docente</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6 px-8 py-7">
      <div className="flex flex-col gap-3">
        <Link href="/profesores" className="flex w-fit items-center gap-1.5 text-[12px] font-medium text-subtle hover:text-ink"><ArrowLeft className="h-3.5 w-3.5" /> Docentes</Link>
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            <h1 className="text-[22px] font-bold -tracking-[0.02em] text-ink">Nuevo docente</h1>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-line bg-card p-6">
        {step === 0 && (
          <>
            ...
            <div className="flex gap-3">
              ...
              <TextField label="Número de documento" required value={p.documentId ?? ""} onChange={set("documentId")} placeholder="71000000" />
            </div>
            <div className="flex gap-3">
              <TextField label="Primer nombre" required value={firstName} onChange={setFirstName} placeholder="Carlos" />
              <TextField label="Segundo nombre" value={p.secondName ?? ""} onChange={set("secondName")} />
            </div>
            <div className="flex gap-3">
              <TextField label="Primer apellido" required value={lastName} onChange={setLastName} placeholder="Mendoza" />
        ...
        {step === 1 && (
          <>
            <div className="flex flex-col gap-1">
              <TextField label="Correo" required type="email" value={email} onChange={setEmail} placeholder="carlos.mendoza@colegio.edu.co" />
              {email.trim() && !EMAIL_RE.test(email.trim()) && <span className="text-[11px] text-s-error-fg">El correo no tiene un formato válido.</span>}
            </div>
            <div className="flex gap-3">
        ...
        {step === 2 && (
          <>
            <TextField label="Especialidad / área" value={speciality} onChange={setSpeciality} placeholder="Matemáticas" />
            <div className="flex flex-wrap gap-1.5">
              {SUGGESTED.map((s) => (
                <button key={s} type="button" onClick={() => setSpeciality(s)}
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${speciality === s ? "border border-primary bg-primary/10 text-primary" : "border border-line text-subtle hover:bg-surface"}`}>{s}</button>
              ))}
            </div>
            <div className="flex gap-3">
              <SelectField label="Nivel educativo" value={p.educationLevel ?? ""} onChange={set("educationLevel")} options={EDUCATION_LEVELS} />
              <TextField label="Escalafón docente" value={p.escalafon ?? ""} onChange={set("escalafon")} placeholder="2D, 14…" />
            </div>
            <TextField label="Título profesional" value={p.title ?? ""} onChange={set("title")} placeholder="Licenciado en Matemáticas" />
            <div className="flex gap-3">
            ...
            <p className="flex items-start gap-2 rounded-lg bg-surface px-3 py-2.5 text-[12px] text-subtle">
              <BookOpen className="mt-0.5 h-3.5 w-3.5 shrink-0" /> Las materias y el horario se asignan después en la sección de Horarios.
            </p>
          </>
        )}
        ...
        {step === 4 && (
          <div className="flex flex-col gap-3 text-[13px]">
            <Row label="Docente" value={`${firstName} ${lastName}`} />
            <Row label="Documento" value={`${p.documentType ?? ""} ${p.documentId ?? ""}`.trim()} />
            <Row label="Correo / celular" value={`${email} · ${mobile}`} />
            <Row label="Especialidad" value={speciality || "—"} />
            <Row label="Nivel / vinculación" value={`${p.educationLevel ?? "—"}${p.contractType ? ` · ${p.contractType}` : ""}`} />
      ...
      <div className="flex items-center justify-between">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <button onClick={submit} disabled={saving}
            className="flex h-10 items-center gap-2 rounded-lg bg-primary px-5 text-[13px] font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Registrar docente</button>
        )}
      </div>
    </div>
  );
